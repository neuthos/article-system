<article class="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
    <div class="relative">
        <a href="{{ route('articles.index') }}"
           class="absolute top-4 left-4 z-10 flex items-center bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full text-gray-700 hover:bg-white transition-colors duration-200">
        ...
        @if($article->image)
            <div class="relative h-96">
                ...
                <div class="absolute bottom-0 left-0 right-0 p-8">
                    <h1 class="text-4xl font-bold text-white mb-4 leading-tight">{{ $article->title }}</h1>
                    ...
                    <div class="flex items-center text-white/90">
                        ...
                        <div class="ml-3">
                            <span class="block font-medium">{{ $article->author }}</span>
                            <time datetime="{{ $article->published_at->toISOString() }}" class="text-sm">
        ...
        @else
            <div class="bg-gradient-to-r from-blue-500 to-purple-600 h-48 flex items-center justify-center">
                <h1 class="text-4xl font-bold text-white px-8 text-center">{{ $article->title }}</h1>
            </div>
            <div class="flex items-center p-6 bg-gray-50 border-b">
                ...
                <div class="ml-3">
                    <span class="block font-medium text-gray-700">{{ $article->author }}</span>
                    <time datetime="{{ $article->published_at->toISOString() }}" class="text-sm text-gray-500">
                        {{ $article->published_at->format('F j, Y') }}
                    </time>
                </div>
            </div>
    ...
    <div class="p-8 md:p-12">
        ...
        <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-8 rounded">
            <p class="text-lg font-medium text-gray-700">{{ $article->summary }}</p>
        </div>
    ...
    <div class="bg-gray-50 p-8 border-t">
        <h3 class="text-lg font-bold text-gray-700 mb-4">Share this article</h3>
